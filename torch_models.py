...
class RamanPredictorCNN(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc1 = nn.Linear(input_size, 128)  # Fully connected layer to project input to 128 features
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, padding=1)
        self.conv3 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=3, padding=1)
        self.fc2 = nn.Linear(64 * input_size, 256)
        self.fc3 = nn.Linear(256, output_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        x = self.relu(self.fc1(x))
        x = x.unsqueeze(1)  # shape is now [batch_size, 1, fc_layer_size (e.g., 128)]
...
class RamanPredictorFCN_ModFirst(nn.Module):
    def __init__(self, input_size, output_size):
        super(RamanPredictorCNN, self).__init__()
